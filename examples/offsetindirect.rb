...
class OffsetindirectShim
  ...
  def get(name)
    return 0 unless Offsetindirect.respond_to?(name)
    ruby_value_from_excel_value(Offsetindirect.send(name))
  end
  ...
  def ruby_value_from_excel_value(excel_value)
    ...
    when :ExcelRange
      ...
      p = excel_value[:array]
      s = Offsetindirect::ExcelValue.size
      a = Array.new(r) { Array.new(c) }
      (0...r).each do |row|
        (0...c).each do |column|
          a[row][column] = ruby_value_from_excel_value(Offsetindirect::ExcelValue.new(p + (((row*c)+column)*s)))
        end
      end 
      return a
  ...
  def set(name, ruby_value)
    ...
    return false unless Offsetindirect.respond_to?(name)
    Offsetindirect.send(name, excel_value_from_ruby_value(ruby_value))
  end

  def excel_value_from_ruby_value(ruby_value, excel_value = Offsetindirect::ExcelValue.new)
    case ruby_value
    when Numeric
    ...
    when Array
      ...
      pointer = FFI::MemoryPointer.new(Offsetindirect::ExcelValue, ruby_values.size)
      excel_value[:array] = pointer
      ruby_values.each.with_index do |v,i|
        excel_value_from_ruby_value(v, Offsetindirect::ExcelValue.new(pointer[i]))
      end
    when Symbol
...
module Offsetindirect
  extend FFI::Library
  ffi_lib  File.join(File.dirname(__FILE__),FFI.map_library_name('offsetindirect'))
  ...
  attach_function 'sheet1_a7', [], ExcelValue.by_value
  attach_function 'sheet1_b7', [], ExcelValue.by_value
  attach_function 'sheet1_c7', [], ExcelValue.by_value
  attach_function 'sheet1_a9', [], ExcelValue.by_value
  attach_function 'sheet1_b9', [], ExcelValue.by_value
  # end of Sheet1
